...
namespace KALPI.Controllers
{
    ...
    public class StateMasterController : Controller
    {
        ...
        [HttpPost]
        public JsonResult Save(ENT.StateMaster model)
        {
            ...
            if (model != null)
            {
                ...
                mRes = new BAL.StateMaster().StateMaster_Insert_Update(model);
            }
        ...
        {
            return Json(new BAL.StateMaster().StateMaster_Get_GetAll(Guid.Parse(id)), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult GeneralAction(string id, Int32 ActionType)
        {
            COM.MEMBERS.SQLReturnValue mRes = new BAL.StateMaster().StateMaster_Delete_IsActive(Guid.Parse(id), ActionType);
            return Json(mRes.Outval, JsonRequestBehavior.AllowGet);
        }
    }
}