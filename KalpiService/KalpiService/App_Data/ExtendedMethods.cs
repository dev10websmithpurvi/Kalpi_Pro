...
public static class ExtendedMethods
{
    ...
    public static string GetUserFullName(this IPrincipal user)
    {
        ...
        {
            MySession.Current.UserProfile = (ENT.UserProfile)new BAL.UserProfile().UserProfile_Get(user.GetLogged_Userid(),string.Empty);
            MySession.Current.UserFullname = MySession.Current.UserProfile.up_name;
        }
        return MySession.Current.UserFullname;
    }

    public static ENT.UserProfile GetUserProfile(this IPrincipal user,string deviceid)
    {
        ENT.UserProfile obj = new ENT.UserProfile();
        Guid usrid = new Guid(user.Identity.GetUserId());
        return (ENT.UserProfile)new BAL.UserProfile().UserProfile_Get(user.GetLogged_Userid(),deviceid);
    }
